...
def quitScreen():
    messagebox.showinfo("collecting data", "點擊視窗並採集八個點")
    root.destroy()
    root2=Tk()
    root2.destroy()
...
def Result_Print():
    ...
    '''
    if boolean == True:
        tab6=Frame(tablayout)
        tab6.pack(fill="both")
        for row in range(len(col)):
            for column in range(1):
                label=Label(tab6,width=22,height=1,text=col[row],bg="black",fg="white",padx=1,pady=1)
                label.grid(row=row,column=0,sticky="nsew",padx=1,pady=1)
                tab1.grid_columnconfigure(column,weight=1)
        for row in range(len(col)):
            for column in range(1):
                label=Label(tab6,width=22,height=1,text=row_text2[row],bg="black",fg="white",padx=1,pady=1)
                label.grid(row=row,column=1,sticky="nsew",padx=1,pady=1)
                tab1.grid_columnconfigure(column,weight=1)
        tablayout.add(tab6,text="最接近配方2")
    '''
    tablayout.pack()
    window.mainloop()
            ...
            if len(refPt)==8:
                ...
                if df3.empty ==True:
                    ...
                    if Zero.empty==False:
                        ...
                        '''
                        elif Neg_filtdf.empty==True and filtdf.empty ==False:
                            filtdf=filtdf.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif','color_x','color_y'],axis=1)
                            name=df3['Serial no'].tolist()
                            rate=df3['color_y'].tolist()
                            col=list(filtdf.columns)
                            ...
                            Result_Print()
                            print("最接近的為1",filtdf.head(1))
                        elif Neg_filtdf.empty==False and filtdf.empty ==True:
                            Neg_filtdf=Neg_filtdf.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif','color_x','color_y'],axis=1)
                            name=df3['Serial no'].tolist()
                            rate=df3['color_y'].tolist()
                            col=list(Neg_filtdf.columns)
                            df3=df3.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif','color_x','color_y'],axis=1)
                            row_df3=df3.iloc[0].tolist()
                            row_df32=df3.iloc[1].tolist()
                            row_df33=df3.iloc[2].tolist()
                            row_text=Neg_filtdf.iloc[0].tolist()
                            Result_Print()
                            print('2')
                            print("最接近的為2",Neg_filtdf.head(1))
                        else:
                            Neg_filtdf=Neg_filtdf.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif','color_x','color_y'],axis=1)
                            filtdf=filtdf.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif'],axis=1)
                            name=df3['Serial no'].tolist()
                            rate=df3['color_y'].tolist()
                            row_text=filtdf.iloc[0].tolist()
                            row_text2=Neg_filtdf.iloc[0].tolist()
                            df3=df3.drop(['R','G','B','dalta','Rdif','Gdif','Bdif','A','S','Adif','Sdif','color_x','color_y'],axis=1)
                            col=list(df3.columns)
                            row_df3=df3.iloc[0].tolist()
                            row_df32=df3.iloc[1].tolist()
                            row_df33=df3.iloc[2].tolist()
                            print("最接近的為",filtdf.head(1),Neg_filtdf.head(1))
                            print("fuck")
                            boolean=True
                            Result_Print()
                        '''
